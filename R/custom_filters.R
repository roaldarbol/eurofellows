...
reactable_select_filter <- function(id, label, shared_data, group, choices = NULL,
...
# Custom Crosstalk multi-select filter. This is a multi-select dropdown that works
# on columns containing multiple values per row (list columns).
reactable_multiselect_filter <- function(id, label, shared_data, group, choices = NULL,
                                         width = "100%", class = "filter-input") {
  values <- shared_data$data()[[group]]
  keys <- shared_data$key()
        ...
        filter: function(selectElement) {
          ...
          if (selectedValues.length === 0) {
            ...
            selectedValues.forEach(value => {
              if (keys[value]) {
                keys[value].forEach(key => selectedKeys.add(key))
              }
            })
            handle.set(Array.from(selectedKeys))
          }
        }
      }
    })()
  ...
  div(
    ...
    tags$select(
      id = id,
      multiple = TRUE,
      onchange = sprintf("window['__ct__%s'].filter(this)", id),
      style = sprintf("width: %s", validateCssUnit(width)),
      lapply(choices, function(value) tags$option(value = value, value))
    ),
    tags$script(HTML(script))
...
reactable_search_filter <- function(id, label, shared_data, group, width = "100%", class = "filter-input") {
...
reactable_range_filter <- function(id, label, shared_data, group, min = NULL, max = NULL,
